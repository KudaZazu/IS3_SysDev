<?php
    ...
    {
        header("Location: resetpwd.html?error=All fields are required!");
        exit();
    }else{//find user
        ...
        $query = "SELECT * FROM users,customer WHERE customer.userID=\"$usr\" AND users.userID=\"$usr\" AND customer.lname=\"$lname\" ";
        ...
        if (mysqli_num_rows($result)==1)//user found 
        {
            ...
            mysqli_close($conn);
            header("refresh:5; url=resetcust.html");
